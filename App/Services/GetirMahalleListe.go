...
type MahalleListe struct{}

type MahalleResponseModel struct {
	MahalleArray []MahalleResponse
}

type MahalleResponse struct {
	KoyAdi          interface{}
	KoyKayitNo      interface{}
	MahalleAdi      interface{}
	BilesenAdi      interface{}
	MahalleKimlikNo interface{}
}

func (Get MahalleListe) Get(w http.ResponseWriter, r *http.Request) {
	params := r.URL.Query()
	ilceNo := params.Get("IlceNo")

	if ilceNo != "" {
		token := GetRequestToken()
		client := &http.Client{}
		var data = strings.NewReader(`ilceKimlikNo=` + ilceNo + `&adresReCaptchaResponse=`)
		req, err := http.NewRequest("POST", "https://adres.nvi.gov.tr/Harita/mahalleKoyBaglisiListesi", data)
		if err != nil {
			log.Fatal(err)
		}
		...
		if len(response) > 0 {
			var mahalleArray []MahalleResponse
			for _, information := range response {
				MahalleArray := MahalleResponse{
					information["koyAdi"].(string),
					information["koyKayitNo"].(float64),
					information["adi"].(string),
					information["bilesenAdi"].(string),
					information["kimlikNo"].(float64),
				}
				mahalleArray = append(mahalleArray, MahalleArray)
			}

			ResponseModel := MahalleResponseModel{
				MahalleArray: mahalleArray,
			}
			ResponseWriter(w, http.StatusOK, "SUCCESS", "Işleminiz başarılı, 'Mahalle' listesi getirildi!", ResponseModel)
		} else {
			ResponseWriter(w, http.StatusBadRequest, "ERROR", "Işleminiz reddedildi, 'IlceNo' verisine ait mahalle bilgisi bulunamadı!", map[string]interface{}{})
		}
	} else {
		ResponseWriter(w, http.StatusBadRequest, "ERROR", "Işleminiz reddedildi, 'IlceNo' verisi belirtiniz!", map[string]interface{}{})
	}
}
